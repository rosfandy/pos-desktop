...
function Sidebar() {
  ...
  return (
    <aside className="w-48 shrink-0 flex flex-col bg-neutral-900 text-neutral-100 select-none">
      {/* App brand */}
      <div className="h-9 flex items-center gap-2 px-3 bg-neutral-950 border-b border-neutral-800">
        ...
        <span className="text-[12px] font-semibold tracking-wide text-white">POS Desktop</span>
      </div>

      {/* Nav */}
      <nav className="flex-1 overflow-y-auto py-1">
        <div className="px-2 py-1.5">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-500 px-2 mb-1">Menu</p>
          {NAV_ITEMS.map(({ to, label, icon: Icon }) => {
            ...
            return (
              <NavLink
                ...
                className={cn(
                  'flex items-center gap-2.5 px-2 py-1.5 rounded text-[12px] font-medium transition-colors mb-0.5',
                  isActive
                    ? 'bg-indigo-600 text-white'
                    : 'text-neutral-400 hover:bg-neutral-800 hover:text-neutral-100'
                )}
              >
                <Icon weight={isActive ? 'fill' : 'regular'} className="w-3.5 h-3.5 shrink-0" />
                {label}
              </NavLink>
            );
          })}
        ...
        <div className="px-2 py-1.5 mt-1 border-t border-neutral-800">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-500 px-2 mb-1">Sistem</p>
          <NavLink
            to="/settings"
            className={({ isActive }) => cn(
              'flex items-center gap-2.5 px-2 py-1.5 rounded text-[12px] font-medium transition-colors',
              isActive
                ? 'bg-indigo-600 text-white'
                : 'text-neutral-400 hover:bg-neutral-800 hover:text-neutral-100'
            )}
          >
            {({ isActive }) => (
              <>
                <Gear weight={isActive ? 'fill' : 'regular'} className="w-3.5 h-3.5 shrink-0" />
                Pengaturan
              </>
            )}
          </NavLink>
...
function StatusBar() {
  ...
  return (
    <div className="h-6 shrink-0 flex items-center px-3 gap-4 bg-neutral-100 border-t border-neutral-200 select-none">
      ...
      <span className="text-[10px] text-neutral-400 tabular-nums">{totalProducts} produk</span>
      <div className="flex-1" />
